...
pub fn render_main(facts: &MainFacts, git: &GitFacts, config: &Config) -> String {
    ...
    config
        ...
        .iter()
        .filter_map(|entries| {
            let segments = build_line(entries, facts, git, config);
...
mod tests {
    ...
    fn the_default_layout_renders_two_lines() {
        ...
        let out = render_main(&MainFacts::default(), &git, &config());
    ...
    fn a_line_whose_segments_all_omit_is_dropped_not_blank() {
        ...
        let out = render_main(&MainFacts::default(), &GitFacts::default(), &config);
    ...
    fn a_layout_with_no_usable_lines_renders_nothing() {
        ...
        assert_eq!(render_main(&MainFacts::default(), &GitFacts::default(), &config), "");
    }
