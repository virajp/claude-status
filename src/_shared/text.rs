...
/// description written by a model, a remote URL, and
...
/// `projectName`, so a cloned repository now reaches the bar through exactly
/// two strings it controls — `projectName` from its repo layer, and the
/// `origin` URL's path from its `.git/config` — rather than the whole of it,
/// including the powerline separators, which sit *outside* every segment's SGR
/// bracket and were the sharpest target. Both are drawn only after this
/// function.
///
/// **The narrowing is not a reason to relax here.** It shrank one input; it
/// removed none. `projectName` and the remote path are still drawn, still
/// attacker-written, and the remaining sources — a branch name, a worktree
/// directory, a model-authored task description — never went through a config
/// layer at all. The filter is what makes all of them safe, and it is the only
/// thing that does.
///
