...
**Merge with the tasks, not by hand.** `mise run code:merge:develop` lands a
feature branch on `develop`; `mise run code:merge:main` lands `develop` on
`main`. Both refuse a dirty or unpushed branch, merge `--no-ff` so the merge is
a commit git can point at, push, and return you to the branch you started on —
including from a linked worktree, where the checkout has to happen in the main
one.
...
| `code:merge:develop` / `code:merge:main` | the merge          | a merge from `main`, or into `main` from anything but `develop` |
